...
test.describe('AI 할 일 플랜', () => {
  ...
  test('375px 폭에서도 미리보기 행이 잘리지 않고 제목 입력칸이 충분히 넓다', async ({ page, request }) => {
    ...
    const titleInput = page.getByLabel(`견적 받기 ${suffix} 제목`)
    await expect(titleInput).toBeVisible()
    const box = await titleInput.boundingBox()
    expect(box).not.toBeNull()
    expect(box!.width).toBeGreaterThanOrEqual(200)

    const dialog = page.getByRole('dialog', { name: 'AI로 계획' })
