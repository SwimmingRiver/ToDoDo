...
1. **KV 카운터는 완전히 원자적이지 않다.** 게이트 체크 이후 쓰기 직전에 최신값을 다시 읽어 latest+1을 쓰지만, KV get/put 자체는 원자적이지 않아 재조회~쓰기 사이 수 ms 창에서 완전히 동시에 끝나는 요청은 하루 한도를 소폭 넘길 수 있다. 목적은 남용 방지이지 정확한 과금이 아니므로 감수한다. **결제(조각 3) 설계 때 Durable Object 전환을 다시 검토한다.**
...
4. **실패한 호출은 사용량 카운터에 잡히지 않는다**(성공만 센다는 결정 때문). 거부/max_tokens/잘못된 출력으로 502가 나도 카운트가 늘지 않으므로, 프리미엄 사용자가 실패를 반복 유발하면 호출당 최대 약 4k 출력 토큰을 한도 없이 소모시킬 수 있다. 프리미엄을 수동으로만 부여하는 지금은 감수하지만, 결제로 프리미엄을 공개 판매하기 전에는 반드시 닫아야 한다(예: 별도의 느슨한 시도 횟수 상한). Anthropic Console의 월 사용 한도(spend limit)를 필수로 설정해 그 전까지의 최후 방어선으로 삼는다.
