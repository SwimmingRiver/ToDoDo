...
`fill`의 정의는 정확히 **"흰색 또는 회색 배경 위에, 글자를 얹지 않고"**다. 두 조건이 다 필요하다.

- 글자를 얹으면: 흰 글자든 검은 글자든 3.39:1이라 텍스트 4.5:1에 미달한다.
- 연한 초록 배경 위에 놓으면: 비텍스트 3:1마저 깨진다. 실측으로 `#E0EDE8`(2.81:1),
  `#D5EDE4`(2.75:1), `#D9ECE4`(2.75:1), `#D1F5E8`(2.89:1) 위에서 미달이다.

현재 `fill` 대상(프로그레스 채움, 색 점, 완료 체크 배경, 스켈레톤, 스피너)은 전부 흰색 또는
`background.secondary`(`#F4F5F6`) 위에 있어 문제가 없다. 이 제약은 앞으로를 위한 것이다.
...
| `#0D5E49` (코드에 이미 있음) | 7.74:1 ✅ | 1.25:1 (너무 미묘) |
...
**`#0D5E49`를 흡수한다.** `todayPage.styles.tsx:51`과 `todoList.styles.tsx:36`이 이미
`primary`(`#0F6E56`) 배경 버튼의 hover로 이 값을 하드코딩해 쓰고 있다 — 코드베이스가 이미
`strong`+`strongHover` 구조를 갖고 있었던 셈이다. 다만 변화폭이 1.25:1로, 지금 대다수 버튼이
쓰는 1.83:1보다 한참 밋밋하다. 이걸 채택하면 나머지 버튼들의 hover 피드백이 후퇴하므로,
반대로 이 2곳을 `strongHover`로 끌어올려 어두운 초록을 하나로 만든다.
...
테두리·밑줄·좌측 바가 **바로 옆 글자와 같은 토큰을 공유하는 한 쌍**이면 글자를 따라
`strong`으로 간다. 한쪽만 남기면 같은 요소 안에서 두 초록이 어긋난다.

| 위치 | 짝 |
|---|---|
| `ctaButtons.styles:40`+`:42` | 보조 CTA 버튼 — 글자 + 테두리 |
| `calendar.styles:195`+`:198` | 캘린더 버튼 — 테두리 + 글자 |
| `kanbanBoard.styles:114`+`:116` | 활성 필터 탭 — 글자 + 밑줄 |
| `recurrence.styles:108`+`:110` | 활성 탭 — 글자 + 밑줄 |
| `snb.tsx:103`+`:108` | 활성 내비 — 글자 + 좌측 바(`#1D9E75` 하드코딩) |
| `mobileDrawer.styles:112`+`:114` | 활성 내비 — 글자 + 좌측 바(`#1D9E75` 하드코딩) |

좌측 바 2곳은 `tint` 배경 위에 놓인다. `fill`로 남기면 `#1D9E75` vs `#E8F4F1`이 3.01:1로
기준선에 걸치고, 옆 글자와도 어긋난다.

### 흰 글자를 얹는 활성 토글

`calendar.styles:161-163`의 `ViewButton`은 `$active`일 때 배경 `secondary` + 글자 `#ffffff`다.
솔리드 버튼과 같은 위반이므로 배경과 테두리 모두 `strong`으로 간다.
...
| `#1D9E75` × 2 | `snb.tsx:108`, `mobileDrawer.styles:114` | border-left 토큰 우회 | `strong` (아래 참고) |
| `#0D5E49` × 2 | `todayPage.styles:51`, `todoList.styles:36` | 하드코딩된 hover — 사실상 `strongHover` | `strongHover` |
...
- **연한 초록 7종 난립**: `#E8F5EF`(토큰), `#E8F4F1`, `#E0EDE8`, `#D5EDE4`, `#D9ECE4`,
  `#F0FBF7`, `#D1F5E8`이 배경·hover로 흩어져 있다. `strong` 글자는 이 **7종 전부에서**
  4.5:1을 통과함을 실측 확인했으므로(5.04~5.86:1) AA를 막지 않는다. 통합은 별도 위생 작업이다.
  단, 위 "하드코딩" 표에 올린 `#E8F4F1`·`#e8f0fe`는 각각 토큰과 거의 같은 값이거나 파란색이라
  이번에 흡수한다.
- `#4CAF50`/`#E8F5E9`(토스트 성공), `#34A853`(구글 로그인 버튼) — 브랜드색이 아닌 별개 체계
