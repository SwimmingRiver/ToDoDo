...
| 확장 시 `getNextRootOrder` | 1 (쓸 때만) | 0 (공유 스냅샷에서 메모리 계산) |
| 무효화로 인한 `getTodos()` 재조회 | 최대 3 | 쓴 게 있을 때만 1 |

- **정상 상태**(쓸 게 없는 대부분의 실행): `4 + N` → **2**
- **쓸 게 있는 실행**: `5 + N + 최대 3` → **3** (확장이 쓰는 경우 `getNextRootOrder` 1회가 더 있었다)

> 구현 후 보강(2026-08-13, 전체 브랜치 리뷰): `getNextRootOrder` 행은 구현 중에 드러난 항목이다.
> 확장 스윕은 공유 스냅샷을 이미 손에 들고도 `startOrder`를 구하려고 루트 컬렉션을 한 번 더
> 읽고 있었다. 그 쿼리(`userId` + `parentId == null`)는 공유 스냅샷을 `parentId === null`로
> 거른 것과 같은 집합이므로 순수한 낭비였다. 이걸 제거하기 전까지 위 "쓸 게 있는 실행 → 3"은
> 실제로 4였다. 지금은 표가 코드와 일치한다.
