...
PR 본문에 이 목록을 그대로 넣는다. main 병합 **전에** 1~4를 전부 끝내야 한다 — 같은 main-push 빌드가 client Hosting을 이 시크릿으로 굽기 때문이다.

1. `cd ai-proxy && npx wrangler kv namespace create AI_USAGE` → id를 `wrangler.toml`에 반영해 커밋
2. Anthropic Console에서 API 키 발급 + 월 사용 한도(spend limit) 설정(**필수** — 실패 호출은 카운트되지 않아 이 한도가 최후 방어선)
3. 본인 터미널에서 `cd ai-proxy && npx wrangler secret put ANTHROPIC_API_KEY`
4. Worker URL 확정(`https://tododo-ai-proxy.<account-subdomain>.workers.dev`로 예측 가능하거나, 확실히 하려면 로컬에서 `npx wrangler deploy`로 먼저 한 번 수동 배포해 출력 확인) 후 GitHub Secret `VITE_AI_PROXY_URL` 등록
5. 로컬 `client/.env`에도 `VITE_AI_PROXY_URL` 추가
6. 배포 후 프리미엄 계정으로 실사용 확인: 목표 입력 → 결과 품질 → 추가 → 캘린더 연동 사용자라면 구글 캘린더 반영

PR 본문 "알고 감수한 한계" 섹션: KV 카운터 비원자성(쓰기 직전 재조회로 창을 줄였으나 완전히 동시인 요청은 여전히 소폭 초과 가능, 결제 도입 시 Durable Object 재검토), 실패한 호출은 카운트되지 않음(결제 공개 전 별도 시도 상한 필요), premium 클레임 전파 최대 1시간 지연, 기존 일정을 보지 않아 바쁜 날을 피하지 못함(범위 결정).
