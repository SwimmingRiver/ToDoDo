...
export declare const toDateKeyFromISO: (iso: string) => string;
/** "yyyy-MM-dd" → 로컬 자정 Date. `new Date("yyyy-MM-dd")`는 UTC 자정이라 쓰지 않는다. */
export declare const parseDateKey: (key: string) => Date;
/** 날짜 키에 일수를 더한다(음수 가능). 월/연 경계는 Date#setDate가 처리한다. */
export declare const addDaysToKey: (key: string, days: number) => string;
/** "yyyy-MM-dd" → "yyyy-MM". */
export declare const toMonthKey: (dateKey: string) => string;
/** "yyyy-MM"에 개월 수를 더한다(음수 가능). */
export declare const addMonthsToMonthKey: (monthKey: string, months: number) => string;
/**
 * 두 날짜 키 사이의 달력 일수(toKey - fromKey). 로컬 자정끼리의 ms 차이를 하루로
 * 나눈 뒤 반올림해서, DST로 23/25시간짜리 날이 끼어도 정수 일수가 유지된다.
 */
export declare const diffDaysBetweenKeys: (fromKey: string, toKey: string) => number;
