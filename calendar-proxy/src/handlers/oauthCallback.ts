...
export const handleOAuthCallback = async (request: Request, env: Env): Promise<Response> => {
  ...
  // state는 /oauth/start가 발급한 1회용 토큰이다 — 여기서 실제 uid·returnOrigin으로
  // 교환하고 즉시 소비한다. uid 자체를 state로 왕복시키면 Authorization 헤더가
  // 없는 이 엔드포인트에서 누구든 다른 사용자의 uid를 흉내 낼 수 있다.
  const consumed = await consumeOAuthState(env.CALENDAR_TOKENS, state);
  if (!consumed) {
    return Response.redirect(`${env.CLIENT_APP_URL}/calendar?calendarError=1`, 302);
  }
  const { uid, returnOrigin } = consumed;
  ...
  try {
    ...
    return Response.redirect(`${returnOrigin}/calendar?calendarConnected=1`, 302);
  } catch (error) {
    console.error("OAuth 콜백 실패:", error);
    return Response.redirect(`${returnOrigin}/calendar?calendarError=1`, 302);
  }
};
